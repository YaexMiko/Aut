...
class Var:
    ...
    BACKUP_CHANNEL = getenv("BACKUP_CHANNEL", "-1001627911624") or ""
    MAIN_CHANNEL = int(getenv("MAIN_CHANNEL", "-1002024030760"))
    LOG_CHANNEL = int(getenv("LOG_CHANNEL") or 0)
    FILE_STORE = int(getenv("FILE_STORE", "-1001627911624"))
    ADMINS = list(map(int, getenv("ADMINS", "831859341 8108281129").split()))
    ...
    FFCODE_1440 = getenv("FFCODE_1440") or """ffmpeg -i '{}' -progress '{}' -preset ultrafast -c:v libx264 -s 2560x1440 -pix_fmt yuv420p -crf 31 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 '{}' -y"""
    QUALS = getenv("QUALS", "1440 480 720 1080").split()
